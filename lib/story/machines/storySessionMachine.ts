 ...
 */
export const storySessionMachine = setup({
  types: {
    context: {} as StorySessionContext,
    events: {} as StorySessionEvent,
    input: {} as StorySessionInput,
  },
  actions: {
    /**
  ...
  guards: {
    ...
    shouldShowFinale: ({ context }) => {
      ...
      const beatsExhausted = context.currentBeatIndex >= context.generatedStory.stage1Beats.length;
    ...
    shouldTransitionToStage2: ({ context }) => {
      ...
      const stage1Complete = context.currentBeatIndex >= context.generatedStory.stage1Beats.length;
