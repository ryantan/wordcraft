...
import type { GameMechanicId, GameResult } from '@/types';
...
export function calculateStylePerformances(results: GameResult[]): StylePerformance[] {
  ...
  styles.forEach(style => {
    ...
    const successRate = (styleResults.filter(r => r.correct).length / styleResults.length) * 100;

    const averageAttempts =
      styleResults.reduce((sum, r) => sum + r.attempts, 0) / styleResults.length;

    const averageTimeMs = styleResults.reduce((sum, r) => sum + r.timeMs, 0) / styleResults.length;
...
export function getRecommendedGames(
  ...
  const gamesByStyle = availableGames.reduce(
    ...
    { visual: [], auditory: [], kinesthetic: [] } as Record<LearningStyleType, GameMechanicId[]>,
  );
