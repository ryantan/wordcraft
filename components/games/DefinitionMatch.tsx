...
export const DefinitionMatch: FC<DefinitionMatchProps> = ({
  ...
  useEffect(() => {
    ...
    const similarWords = extraWordInfo.similar_words.filter(
  ...
  const definition = extraWordInfo?.meaning || 'Select the correct word';
      ...
      <div className="max-w-2xl w-full space-y-8">
        ...
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-bold text-gray-900">
            Which word or phrase does this sentence describe?
          </h2>
        </div>
        ...
        <div className="bg-primary-50 rounded-xl p-6 border-2 border-primary-200">
          ...
          <p className="text-lg text-center text-gray-800 font-medium">{definition}</p>
        </div>

        {/* Word Options */}
        <div className="space-y-3">
          <p className="text-gray-600 text-center">
            Read the definition and select the word or phrase that matches
          </p>
        ...
        <div className="flex gap-3 justify-center">
          {onHintRequest && (
            <Button
              ...
              onClick={handleHint}
              disabled={
                showFeedback || eliminatedOptions.size >= options.length - 1 || options.length <= 2
              }
            >
              💡 Hint ({hintsUsed})
            </Button>
